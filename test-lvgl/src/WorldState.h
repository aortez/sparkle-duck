...
struct WorldState {
    ...
    struct CellData {
        ...
        CellData()
            : material_mass(0.0),
              dominant_material(MaterialType::AIR),
              velocity(0.0, 0.0),
              com(0.0, 0.0)
        {}
        ...
        CellData(
            ...
            const Vector2d& vel = Vector2d(0.0, 0.0),
            const Vector2d& center_of_mass = Vector2d(0.0, 0.0))
            : material_mass(mass), dominant_material(material), velocity(vel), com(center_of_mass)
        {}
    ...
    WorldState()
        : width(0),
          ...
          pressure_scale(1.0),
          dirt_fragmentation_factor(1.0),
          water_pressure_threshold(0.1),
          left_throw_enabled(false),
          right_throw_enabled(false),
          lower_right_quadrant_enabled(false),
          walls_enabled(true),
          rain_rate(0.0),
     ...
     */
    const CellData& getCellData(uint32_t x, uint32_t y) const { return grid_data[y][x]; }

    /**
     * Set cell data at specified coordinates
     */
    void setCellData(uint32_t x, uint32_t y, const CellData& data) { grid_data[y][x] = data; }

    /**
     * Check if coordinates are valid
     */
    bool isValidCoordinate(uint32_t x, uint32_t y) const { return x < width && y < height; }
