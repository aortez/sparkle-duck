...
DirtSimStateMachine::DirtSimStateMachine(lv_disp_t* display) 
    ...
    if (display) {
        ...
        spdlog::info("DirtSimStateMachine initialized with UI support in state: {}", getCurrentStateName());
    } else {
        spdlog::info("DirtSimStateMachine initialized in headless mode in state: {}", getCurrentStateName());
    }
}
...
void DirtSimStateMachine::processImmediateEvent(const Event& event, SharedSimState& shared) {
    ...
    std::visit([this, &shared](const auto& e) {
...
State::Any DirtSimStateMachine::onEvent(const GetFPSCommand& /*cmd*/) {
    ...
    return std::visit([](auto&& state) -> State::Any {
