...
#define SPARKLE_ASSERT_MSG(condition, format, ...)                                              \
    do {                                                                                        \
        if (!(condition)) {                                                                     \
            ...
            snprintf(                                                                           \
                _sparkle_assert_buffer, sizeof(_sparkle_assert_buffer), format, ##__VA_ARGS__); \
            CrashDumpHandler::onAssertionFailure(                                               \
                #condition, __FILE__, __LINE__, _sparkle_assert_buffer);                        \
            assert(condition);                                                                  \
        }                                                                                       \
    } while (0)
 ...
 */
#define SPARKLE_DUMP(reason) CrashDumpHandler::dumpWorldState(reason)
...
#define ASSERT(condition, message, ...) SPARKLE_ASSERT_MSG(condition, message, ##__VA_ARGS__)
