...
void EventProcessor::queueEvent(const Event& event) {
    ...
    spdlog::debug("EVENT_QUEUED: {} [queue_depth: {}, thread: {}]", 
                  getEventName(event), eventQueue.size() + 1, 
                  oss.str());
...
void EventProcessor::processEvent(const Event& event, DirtSim::DirtSimStateMachine& dsm) {
    ...
    spdlog::info("Processing event: {} in state: {}", 
                 getEventName(event), dsm.getCurrentStateName());
    ...
    try {
        ...
        spdlog::error("Exception processing event {}: {}", 
                      getEventName(event), e.what());
