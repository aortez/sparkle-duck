 ...
 * selected */
#if LV_USE_SDL == 0 && LV_USE_WAYLAND == 0 && LV_USE_LINUX_DRM == 0 &&         \
    LV_USE_OPENGLES == 0 && LV_USE_X11 == 0 && LV_USE_LINUX_FBDEV == 0
...
static backend_t
    *backends[sizeof(available_backends) / sizeof(available_backends[0])];
...
void driver_backends_register(void) {
  ...
  while ((init_backend = available_backends[i]) != NULL) {
    ...
    b->handle =
        static_cast<backend_handle_t *>(malloc(sizeof(backend_handle_t)));
...
int driver_backends_init_backend(char *backend_name) {
  ...
  if (backend_name == NULL) {
    ...
    if (b->type != BACKEND_DISPLAY) {
      LV_LOG_ERROR("The default backend: %s is not a display driver backend",
                   b->name);
      return -1;
    }
    ...
    if (strcmp(b->name, backend_name) == 0) {
      ...
      if (b->type == BACKEND_DISPLAY) {
        ...
        if (sel_display_backend == NULL) {
          LV_LOG_ERROR("Failed to init indev backend: %s - display needs to be "
                       "initialized",
                       b->name);
          return -1;
        }
...
void driver_backends_run_loop(World &world) {
  ...
  if (sel_display_backend != NULL &&
      sel_display_backend->handle->display != NULL) {
