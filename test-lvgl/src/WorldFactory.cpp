...
std::unique_ptr<WorldInterface> createWorld(
    ...
    switch (type) {
        ...
        default:
            throw std::runtime_error("Unknown WorldType: " + std::to_string(static_cast<int>(type)));
    }
}
...
WorldType parseWorldType(const std::string& typeStr) {
    ...
    } else if (lowerStr == "rulesb" || lowerStr == "rules_b" || lowerStr == "b" || lowerStr == "worldb") {
        return WorldType::RulesB;
    } else {
        throw std::runtime_error("Unknown world type string: '" + typeStr + "'. Valid options: rulesA, rulesB");
    }
}