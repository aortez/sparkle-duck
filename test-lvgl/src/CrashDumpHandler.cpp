...
void CrashDumpHandler::onAssertionFailure(const char* condition, const char* file, int line, const char* message)
{
    if (!installed_ || !manager_) {
        spdlog::error("ASSERTION FAILURE: {} at {}:{} - {}", condition, file, line, message ? message : "");
...
void CrashDumpHandler::writeWorldStateToFile(const std::string& filename, const char* reason,
                                           ...
                                           const char* message)
{
    try {
        ...
        if (condition) {
            crashInfo.AddMember("assertion_condition", rapidjson::Value(condition, allocator), allocator);
        }
        if (file) {
        ...
        if (message) {
            crashInfo.AddMember("assertion_message", rapidjson::Value(message, allocator), allocator);
        }
        ...
        switch (world->getWorldType()) {
            case WorldType::RulesA: worldTypeName = "RulesA"; break;
            case WorldType::RulesB: worldTypeName = "RulesB"; break;
        }
...
void CrashDumpHandler::logDumpSummary(const std::string& filename, const char* reason)
{
    ...
    spdlog::info("World: {}x{} cells, {} timesteps", 
                 world->getWidth(), world->getHeight(), world->getTimestep());
    spdlog::info("Mass: {:.3f} total, {:.3f} removed", 
                 world->getTotalMass(), world->getRemovedMass());
