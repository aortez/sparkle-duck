...
WorldState WorldState::fromJson(const rapidjson::Value& json)
{
    ...
    if (!json.HasMember("grid") || !json.HasMember("physics") || 
        !json.HasMember("setup") || !json.HasMember("cells")) {
    ...
    if (!grid.IsObject() || !grid.HasMember("width") || !grid.HasMember("height") || !grid.HasMember("timestep")) {
    ...
    if (physics.IsObject()) {
        ...
        if (physics.HasMember("dirt_fragmentation_factor") && physics["dirt_fragmentation_factor"].IsNumber()) {
            state.dirt_fragmentation_factor = physics["dirt_fragmentation_factor"].GetDouble();
        }
        if (physics.HasMember("water_pressure_threshold") && physics["water_pressure_threshold"].IsNumber()) {
    ...
    if (setup.IsObject()) {
        ...
        if (setup.HasMember("lower_right_quadrant_enabled") && setup["lower_right_quadrant_enabled"].IsBool()) {
    ...
    for (rapidjson::SizeType i = 0; i < cells.Size(); ++i) {
        ...
        if (!cellEntry.IsObject() || !cellEntry.HasMember("x") || 
            !cellEntry.HasMember("y") || !cellEntry.HasMember("data")) {
            throw std::runtime_error("WorldState::fromJson: Invalid cell entry format");
        }
        
        if (!cellEntry["x"].IsUint() || !cellEntry["y"].IsUint()) {
            throw std::runtime_error("WorldState::fromJson: Cell coordinates must be positive integers");
        }
