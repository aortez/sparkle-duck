...
TEST_F(WorldBVisualTest, VelocityLimiting) {
    ...
    // Test that World implements velocity limiting as designed.
    ...
    // World should handle velocity limiting internally.
    // We can't directly test velocities since they're internal to Cell.
...
TEST_F(WorldBVisualTest, VelocityBehaviorTimestepCorrectness) {
    ...
    // we need to cast to World for this specific test.
    if (auto* worldB = dynamic_cast<World*>(world.get())) {
    ...
    // This test needs direct access to World implementation details.
    auto* worldB = dynamic_cast<World*>(world.get());
    ASSERT_NE(worldB, nullptr) << "This test requires World implementation";
    
    // Get initial cell and set a known velocity.
    Cell& startCell = worldB->at(0, 0);
    ...
    for (stepsToFirstTransfer = 0; stepsToFirstTransfer < expectedStepsToFirstBoundary * 2; ++stepsToFirstTransfer) {
        ...
        Cell& firstCell = worldB->at(0, 0);
        Cell& secondCell = worldB->at(1, 0);
...
TEST_P(CollisionBehaviorTest, MaterialCollisionBehavior) {
    ...
    for (int step = 0; step < maxSteps; step++) {
        ...
        Cell& cell0 = worldB->at(0, 0);
        Cell& cell1 = worldB->at(1, 0);
        Cell& cell2 = worldB->at(2, 0);
