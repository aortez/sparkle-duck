...
// Forward declaration
namespace DirtSim { class DirtSimStateMachine; }
...
public:
     ...
     */
    static State::Any dispatch(State::Any currentState, 
                              const Event& event,
                              DirtSimStateMachine& dsm) {
        return std::visit([&event, &dsm](auto&& state) -> State::Any {
            using StateType = std::decay_t<decltype(state)>;
            
            return std::visit([&state, &dsm](auto&& evt) -> State::Any {
                ...
                if constexpr (hasEventHandler<StateType, EventType>()) {
                    spdlog::info("Dispatching {} to state handler in {}", 
                                 EventType::name(), StateType::name());
                    return state.onEvent(evt, dsm);
                }
                // Try global handler
                else if constexpr (hasGlobalHandler<EventType>()) {
                    spdlog::info("Dispatching {} to global handler from {}", 
                                 EventType::name(), StateType::name());
                    return dsm.onEvent(evt);
                }
                // No handler found
                else {
                    spdlog::info("No handler for {} in state {}", 
                                 EventType::name(), StateType::name());
                    return state;
                }
            }, event);
