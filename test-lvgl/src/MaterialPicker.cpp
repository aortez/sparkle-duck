...
void MaterialPicker::createMaterialSelector()
{
    ...
    lv_obj_set_style_grid_column_dsc_array(material_grid_, 
        (const lv_coord_t[]){BUTTON_SIZE, BUTTON_SIZE, LV_GRID_TEMPLATE_LAST}, 0);
    lv_obj_set_style_grid_row_dsc_array(material_grid_, 
        (const lv_coord_t[]){BUTTON_SIZE, BUTTON_SIZE, BUTTON_SIZE, BUTTON_SIZE, LV_GRID_TEMPLATE_LAST}, 0);
...
void MaterialPicker::createMaterialButton(MaterialType type, int gridX, int gridY)
{
    ...
    spdlog::trace("Creating material button for {} at grid position ({},{}), index {}",
                  getMaterialName(type), gridX, gridY, index);
    ...
    spdlog::trace("Created material button for {} with user_data={}", 
                  getMaterialName(type), static_cast<int>(type));
}
...
void MaterialPicker::setSelectedMaterial(MaterialType type)
{
    if (selected_material_ != type) {
        spdlog::debug("Material selection changed: {} -> {}", 
                      getMaterialName(selected_material_), getMaterialName(type));
...
void MaterialPicker::onMaterialButtonClicked(lv_event_t* e)
{
    ...
    spdlog::info("Material button clicked: {} (raw data: {})", 
                 getMaterialName(clickedMaterial), static_cast<int>(clickedMaterial));
...
{
    spdlog::trace("Updating button highlights for selected material: {}", getMaterialName(selectedType));
...
void MaterialPicker::createMaterialIcon(lv_obj_t* button, MaterialType type)
{
    ...
    spdlog::trace("Created {}x{} icon for {} with color and text '{}'", 
                  ICON_SIZE, ICON_SIZE, getMaterialName(type), iconText);
}
